...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setPopupMessage(`You have been granted a ${permission} account.`);
        setShowPopup(true);

        // After 2 seconds, redirect to the login page with autofilled details
        setTimeout(() => {
          navigate('/login', { state: { ...formData, permission } });
        }, 2000);
      }
    } catch (error) {
  ...
  const handlePermissionChange = (selectedPermission) => {
    ...
    setPopupMessage(`You have selected ${selectedPermission} account.`);
    setShowPopup(true);
    
    // After 2 seconds, redirect to login page
    setTimeout(() => {
      navigate('/login', { state: { ...formData, permission: selectedPermission } });
    }, 2000);
  };

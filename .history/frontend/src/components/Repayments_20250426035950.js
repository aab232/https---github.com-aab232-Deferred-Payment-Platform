...
const Repayments = () => {
    ...
    const handleRepaymentSubmit = async (orderId) => {
        ...
        // Call backend API to process repayment
        ...
        if (response.ok && resultData.success) {
            // Show success message for this specific order
            ...
            console.log("Repayment backend successful, re-fetching active orders...");
            const isMountedRef = { current: true }; // Need ref again for async update
            await fetchOrders(isMountedRef); // Re-fetch data to update UI state
            // The input field amount (repaymentAmounts[orderId]) will reset because fetchOrders resets it
        } else {
            console.error(`Repayment failed for order ${orderId}:`, resultData);
            // Show error message for this specific order

  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        // Generate random password and append permission to email
        const generatedPassword = Math.random().toString(36).slice(-8); // Random 8-character password
        const generatedEmail = `${formData.email}_${permission}@example.com`; // Modify email format if needed

        // Redirect to login with generated credentials
        navigate('/login', {
          state: {

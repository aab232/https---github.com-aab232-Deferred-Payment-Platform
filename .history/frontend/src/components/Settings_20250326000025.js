...
const Settings = () => {
  ...
  const updateDetails = async (field) => {
    try {
      ...
      alert(
        `Error updating ${field}: ${error.response?.data?.message || 'Unknown error'}`
      );
    }
  };
  ...
  const updatePassword = async () => {
    ...
    try {
      ...
      alert(
        `Error updating password: ${error.response?.data?.message || 'Unknown error'}`
      );
    }
  };
  ...
  const deleteAccount = async () => {
    if (
      ...
      try {
        ...
        alert(
          `Error deleting account: ${error.response?.data?.message || 'Unknown error'}`
        );
      }
    }
  };

...
app.put('/update-details', async (req, res) => {
  ...
  const query = `UPDATE users SET ${field} = ? WHERE ${field} = ?`;
  ...
  db.query(query, [newValue, oldValue], (err, result) => {
      ...
      if (result.affectedRows === 0) {
          return res.status(404).json({ message: `No matching record for ${field}` });
      }

      res.status(200).json({ message: `${field.replace('_', ' ')} updated successfully!` });
  });
});

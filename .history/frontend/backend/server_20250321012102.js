...
app.put('/update-details', (req, res) => {
  const { email, phone_number, credit_score, bank_account_link } = req.body;
  const sql = `UPDATE users SET email=?, phone_number=?, credit_score=?, bank_account_link=? WHERE email=?`;
  db.query(sql, [email, phone_number, credit_score, bank_account_link, email], (err) => {
    if (err) return res.status(500).json({ message: 'Error updating details' });
...
app.put('/update-password', async (req, res) => {
  ...
  const [user] = await db.promise().query('SELECT password FROM users WHERE email = ?', [email]);

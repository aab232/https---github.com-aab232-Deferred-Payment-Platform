...
app.post('/api/confirm_bnpl_order', authenticateUser, async (req, res) => {
    ...
    if (!product?.numericPrice || !product?.title || typeof term !== 'number' || term <= 0 /*|| !paydayPreference*/ ) { // Decide if paydayPref is required
        console.error("Order confirm failed: Invalid input.", { product, term, paydayPreference });
    ...
    const isValidDate = paydayPreference ? /^\d{4}-\d{2}-\d{2}$/.test(paydayPreference) : true;
    if (!isValidDate) {
        return res.status(400).json({ success: false, message: 'Invalid payday format (YYYY-MM-DD required).' });
    }
    ...
    try {
        ...
        const now = new Date(); const firstDueDate = new Date(now.setMonth(now.getMonth() + 1)).toISOString().split('T')[0];
        ...
        const orderData = {
            ...
            payday_preference: paydayPreference || null
        };
        // Use explicit column list for INSERT
        const orderSql = 'INSERT INTO orders (user_id, assessment_id, product_title, product_price, loan_amnt, selected_term_months, remaining_balance, order_status, next_payment_due_date, order_timestamp, payday_preference) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        const orderValues = [ orderData.user_id, orderData.assessment_id, orderData.product_title, orderData.product_price, orderData.loan_amnt, orderData.selected_term_months, orderData.remaining_balance, orderData.order_status, orderData.next_payment_due_date, orderData.order_timestamp, orderData.payday_preference ];
        const [orderResult] = await connection.query(orderSql, orderValues);
        const orderId = orderResult.insertId;
        console.log(`✅ BNPL Order (ID: ${orderId}) created with preferred payday: ${orderData.payday_preference}.`);

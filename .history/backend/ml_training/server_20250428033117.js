...
function mapScoreAndApplyAdjustments(riskScore, defaultFlag, employmentStatusDB, utilizationRatioRaw) {
    ...
    const defaultFlagIsY = (defaultFlag === 'Y'); // Convert flag to boolean for easier checks
    ...
    if (riskScore === null || typeof riskScore !== 'number' || isNaN(riskScore)) {
        ...
        entitlements = { tier: baseTier, limit: baseLimit, terms: baseTerms, error: "Invalid score" };
         console.log(`   -> Base Entitlements (Error): Tier ${baseTier}, Limit ${baseLimit.toFixed(2)}`);
         return entitlements;
    }

    const TIER1_MAX = 0.14; const TIER2_MAX = 0.28; const TIER3_MAX = 0.42;
    const TIER4_MAX = 0.56; const TIER5_MAX = 0.70; const TIER6_MAX = 0.84; // Threshold for Tier 7 start
    ...
    else                              { baseTier = 7; baseLimit = 150.00;  baseTerms = [3]; } // Tier 7 (Highest Risk)

    console.log(`   -> Base Entitlements: Tier ${baseTier}, Limit ${baseLimit.toFixed(2)}, Terms [${baseTerms.join(',')}]`);
    finalLimit = baseLimit; // Start adjustments from base limit

    // --- Step 2: Apply Adjustments based on Business Rules ---

    // Adjustment 1: Default Penalty (25% reduction - Applied First)
    if (defaultFlagIsY) {
        const reduction = finalLimit * 0.25;
        finalLimit -= reduction;
        adjustments.push(`Default Flag (-${reduction.toFixed(2)})`);
    }

    // Adjustment 2: Employment Status Penalty
    const isUnemployedBasedOnMap = (mapEmploymentStatus(employmentStatusDB) === 'No');
    if (isUnemployedBasedOnMap) {
        let reductionPercent = 0.20; // Base 20% reduction for unemployment
        // Check if also high risk tier (6 or 7) for additional penalty
        if (baseTier >= 6) {
            reductionPercent += 0.10; // Add 10% more
            adjustments.push(`Employment (Unemployed/Other) & High Risk Tier Penalty (-${(reductionPercent * 100).toFixed(0)}%)`);
        } else {
             adjustments.push(`Employment (Unemployed/Other) Penalty (-${(reductionPercent * 100).toFixed(0)}%)`);
        }
        const reduction = finalLimit * reductionPercent;
        finalLimit -= reduction;
    }

    // Adjustment 3: High Utilization (Bonus if no default, Penalty if default)
    const HIGH_UTIL_THRESHOLD = 0.70; // Example: > 70%
    const utilization = isNaN(Number(utilizationRatioRaw)) ? 0 : Number(utilizationRatioRaw); // Ensure number

    if (utilization > HIGH_UTIL_THRESHOLD) {
        if (defaultFlagIsY) {
            // High utilization AND default -> Decrease limit by 15%
            const reduction = finalLimit * 0.15;
            finalLimit -= reduction;
            adjustments.push(`High Util & Default (-${reduction.toFixed(2)})`);
        } else {
            // High utilization AND NO default -> Increase limit by 20% (up to cap)
            const MAX_LIMIT_CAP = 3000.00; // Example overall cap
            const bonus = finalLimit * 0.20;
            const potentialLimit = finalLimit + bonus;
            const actualBonus = Math.min(bonus, MAX_LIMIT_CAP - finalLimit); // Don't exceed cap
    ...
    // Ensure final limit is not below minimum
    const MINIMUM_LIMIT = 50.00; // Define minimum allowable limit
    finalLimit = Math.max(MINIMUM_LIMIT, finalLimit);
    if (finalLimit < MINIMUM_LIMIT) {
        adjustments.push(`Limit adjusted up to Minimum (${MINIMUM_LIMIT.toFixed(2)})`);
    }
    ...
    // Return final adjusted limit and original tier/terms
    return {
        tier: baseTier,
        limit: parseFloat(finalLimit.toFixed(2)), // Ensure 2 decimal places
        terms: baseTerms,
        error: null
    };
}

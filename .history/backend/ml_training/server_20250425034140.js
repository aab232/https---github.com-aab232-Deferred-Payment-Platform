...
app.post('/api/assess_credit', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        // catch (dbError) { console.error(`   ⚠️ DB Error checking token (User ${userId}):`, dbError); } // Commented out redundant check as it happens below
        ...
        try {
            ...
            // Fetch needed user data fields from users table
            const userSql = `
            ...
            const creditData = latestCreditData || {}; // Use empty object if no credit history
            if (!dbUserData) throw new Error("User profile data missing."); // User must exist if authenticated

            console.log(`      CHECK Type of creditData.person_income: ${typeof creditData.person_income}`);
            console.log(`      CHECK Value of creditData.person_income:`, creditData.person_income);

            // Determine final values for calculation
            // For income, prioritize user table value? Or latest credit snapshot? Using latestCreditData here.
            ...
            // Assemble the features object for Python model (Keys MUST match Python script's expected input)
            rawFeaturesForModel = {
                'employment_status': mapEmploymentStatus(creditData.employment_status || dbUserData.user_employment_status), // Map correct source
                'person_income': final_person_income,
                'cb_person_default_on_file': dbUserData.cb_person_default_on_file || 'N', // Get from users table
                'cb_person_cred_hist_length': Number(dbUserData.cb_person_cred_hist_length || 0), // Get from users table

                'original_loan_amount': Number(creditData.original_loan_amount || 0), // From credit data alias
                'loan_term': final_loan_term, // Based on request or historical
                'loan_amnt': final_loan_amnt, // Based on request or historical

                'credit_utilization_ratio': calculated_util_ratio_db, // From helper (DB based)
                'payment_history': calculated_payment_history_db, // From helper (DB based)
                'loan_percent_income': calculate_lpi(final_loan_amnt, final_person_income) ?? 1.0, // Recalculate, default high
            };
        ...
        if (!entitlements.error) {
             try { const sql = `INSERT INTO credit_assessments (user_id, risk_score, credit_tier, credit_limit, calculated_terms, assessment_timestamp) VALUES (?, ?, ?, ?, ?, NOW())`; const termsJson = JSON.stringify(entitlements.terms||[]); const [insResult] = await dbPool.query(sql, [userId, riskScore, entitlements.tier, entitlements.limit, termsJson]); assessmentIdForOrder = insResult.insertId; console.log(`      -> Record ${assessmentIdForOrder} stored.`); }
             catch (e) { console.error("   ❌ DB Error storing assessment:", e); }
        } else { console.warn(`   ⚠️ Skipping assessment storage: Invalid score/entitlement error.`); }

...
app.post('/api/make_repayment', authenticateUser, async (req, res) => {
    ...
    console.log("   Awaiting DB queries...");
    // --- Execute queries separately ---
    const [orderRows] = await connection.query(getOrderSql, [orderIdInt]); // Result is [rows, fields]
    const [userRows] = await connection.query(getUserSql, [userId]);
    console.log("   DB queries finished.");

    // --- Explicitly check results ---
    console.log("   Order Query Results (Rows Array):", orderRows);
    console.log("   User Query Results (Rows Array):", userRows);

    const order = orderRows?.[0]; // Get first row if orderRows array exists and has element 0
    const user = userRows?.[0];   // Get first row if userRows array exists and has element 0

    // Check if order and user objects were successfully extracted
    if (!order) {
        console.error(`   CRITICAL: Order ID ${orderIdInt} not found in DB result.`);
        throw new Error(`Order ID ${orderIdInt} not found.`);
    }
    if (!user) {
         console.error(`   CRITICAL: User ID ${userId} not found in DB result.`);
        throw new Error(`User ID ${userId} not found for repayment update.`);
    }

    console.log("   Successfully extracted order object:", order);
    console.log("   Successfully extracted user object:", user);

    // --- Now perform the checks (KEEP THE DETAILED CHECK LOGS) ---
    console.log(`   CHECK: Order User ID = ${order.user_id}, Type: ${typeof order.user_id}`); // Should now work
    console.log(`   CHECK: Authenticated User ID = ${userId}, Type: ${typeof userId}`);
    console.log(`   CHECK: Comparison (Number(order.user_id) !== Number(userId)) = ${Number(order.user_id) !== Number(userId)}`);

    // 3. Authorization/Validation Checks
    if (Number(order.user_id) !== Number(userId)) { // Added Number() for safety
        console.error(`Authorization Failed: Order User ID (${order.user_id}) != Auth User ID (${userId})`);

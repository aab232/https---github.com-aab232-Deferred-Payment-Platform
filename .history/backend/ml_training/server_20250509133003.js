...
app.get('/api/spending-limit', authenticateUser, async (req, res) => {
    const userId = req.user.id;
    console.log(`\n⚙️ Fetching spending limit for User ID: ${userId}`);
    try {
        // We need the user's overall current_credit_limit and their self-set monthly_spending_limit
        const sql = 'SELECT current_credit_limit, monthly_spending_limit FROM users WHERE user_id = ?';
        const [userRows] = await dbPool.query(sql, [userId]);
        ...
        const overallCreditLimit = parseFloat(userData.current_credit_limit || 0);
        // monthly_spending_limit can be NULL in DB, so handle that
...
app.post('/api/spending-limit', authenticateUser, async (req, res) => {
    ...
    let newLimitValue = null; // Default to NULL (removing the limit)
    if (spendingLimit !== null) {
        newLimitValue = parseFloat(spendingLimit);
    ...
    try {
        // First, get the user's overall current_credit_limit to validate against
        const [userRows] = await dbPool.query('SELECT current_credit_limit FROM users WHERE user_id = ?', [userId]);
        if (userRows.length === 0) {
            return res.status(404).json({ success: false, message: "User not found." });
        }
        const overallCreditLimit = parseFloat(userRows[0].current_credit_limit || 0);

        if (newLimitValue !== null && newLimitValue > overallCreditLimit) {
            return res.status(400).json({
                success: false,
                message: `Monthly spending limit (£${newLimitValue.toFixed(2)}) cannot exceed your overall credit limit of £${overallCreditLimit.toFixed(2)}.`
            });
        }

        // Update the monthly_spending_limit
        // If newLimitValue is null, it will set the DB field to NULL
        ...
        if (updateResult.affectedRows === 0) {
            // This case should ideally be caught by user not found earlier, but good to have.
        ...
        res.status(200).json({
            ...
            newSpendingLimit: newLimitValue !== null ? newLimitValue.toFixed(2) : null
        });

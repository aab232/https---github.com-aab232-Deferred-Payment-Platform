...
const plaidClient = new PlaidApi(plaidConfig); // initializes the plaid api client
// --- ------------------------ ---
...
function mapScoreAndApplyAdjustments(riskScore, defaultFlag, employmentStatusDB, utilizationRatioRaw) {
    ...
    finalLimit = baseLimit; // initialize final limit

    // applies specific adjustments
    if (defaultFlagIsY) { finalLimit *= 0.75; adjustments.push('Default Flag (-25%)'); } // reduce if default flag

    const empStatusUpper = String(employmentStatusDB || '').toUpperCase(); // normalize employment status
    ...
    const HIGH_UTIL_THRESHOLD = 0.70; // threshold for high credit utilization
    const utilization = isNaN(Number(utilizationRatioRaw)) ? 0 : Number(utilizationRatioRaw); // parse utilization safely
    if (utilization > HIGH_UTIL_THRESHOLD) { // if utilization is high
...
function mapEmploymentStatus(dbStatus) {
    ...
    const statusUpper = String(dbStatus).toUpperCase(); // normalize for comparison
...
app.post('/api/create_link_token', authenticateUser, async (req, res) => {
    ...
    console.log(`⚙️ Creating Plaid link token request for User: ${userId}`);
    ...
    try {
        ...
        if (!entitlements.error && riskScore !== null) { // only store if valid
            ...
            try {
                ...
                throw dbStoreError; // rethrow to main try...catch
            } finally {
...
app.post('/api/make_repayment', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        // 1. log the repayment itself as a transaction record
        ...
        // 2. update the order's remaining balance and status
        ...
        // 3. update the user's total 'used_credit_amount'
        ...
        // 4. process buffer bag contribution

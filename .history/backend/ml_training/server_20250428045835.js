...
app.post('/api/make_repayment', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        // 2. Get/Lock order and user data
        const getOrderSql = 'SELECT user_id, remaining_balance, order_status, product_title FROM orders WHERE order_id = ? FOR UPDATE'; // Added product_title for description
        ...
        // Check if order and user were found
        if (!order) throw new Error(`Order ID ${orderIdInt} not found.`);
        if (!user) throw new Error(`User ID ${userId} not found for repayment.`);

        // 3. Authorization/Validation Checks
        if (Number(order.user_id) !== Number(userId)) throw new Error("User does not own this order."); // Ensure numbers compared
        ...
        // --->>> NEW Step: Insert into transactions table <<<---
        console.log(`   Inserting repayment record into transactions table...`);
        const transactionDescription = `BNPL Repayment for Order #${orderIdInt} (${order.product_title || 'N/A'})`; // Create a description
        const transactionSql = `
            INSERT INTO transactions
            (user_id, transaction_type, amount, transaction_status, description, is_buffer_transaction, transaction_date)
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        `;
        const transactionValues = [
            userId,         // user_id
            'REPAYMENT',    // transaction_type
            amount,         // amount paid
            'Completed',    // transaction_status (since paymentSuccessful is true)
            transactionDescription,
            0               // is_buffer_transaction (assuming 0 for standard repayment)
            // transaction_date uses NOW() / DEFAULT CURRENT_TIMESTAMP
        ];
        try {
             const [transResult] = await connection.query(transactionSql, transactionValues);
             console.log(`   ✅ Transaction record created (ID: ${transResult.insertId})`);
        } catch (transError){
            console.error(`   ❌ Error inserting repayment transaction log:`, transError)
            // Decide: Should failure to log transaction rollback the entire repayment?
            // For auditing, it probably should.
            throw new Error(`Failed to record transaction log: ${transError.message}`);
        }
        // --->>>-------------------------------------- <<<---
        ...
        // 5. Update User's Used Credit Amount
        const currentUsedAmount = parseFloat(user.used_credit_amount);
        const amountToDecrease = Math.min(amount, currentUsedAmount);
    ...
    } catch (error) { // Catches errors from validation, payment sim, DB updates, or transaction log insert
        ...
        // Send back the specific error message that caused the failure
